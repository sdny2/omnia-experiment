...
# 샘플 단위 저장용 함수 
def append_jsonl(rows, path: str):
...
# sample-wise로 stage 1 돌리는 함수 
def run_single_work_item_stage1_samplewise(input_path: str, output_path: str, mode: str):
    # 1) input 로드
    js_list = utils.read_jsonl(input_path)
    contexts = [item["context"] for item in js_list]

    # 2) resume: 이미 저장된 라인 수만큼 스킵
    done = 0
    if os.path.exists(output_path):
        done = len(utils.read_jsonl(output_path))

    fn = _invoke_complete if mode == "complete" else _invoke_chat

    # 3) 샘플 하나씩 호출 + 즉시 append 저장
    for ctx in tqdm(contexts[done:], desc=f"Stage1 sample-wise -> {os.path.basename(output_path)}"):
        raw = fn(ctx)
        formatted = format_result_with_tokens(raw)
        append_jsonl([formatted], output_path)
...
if __name__ == "__main__":
    ...
    # datasets/settings는 반드시 CLI로 받는다 (스크립트 실행 가정)
    if args.datasets is None:
        raise ValueError("Please pass --datasets ...")
    dataset_name_list = args.datasets 

    if args.settings is None:
        raise ValueError("Please pass --settings ...")
    ...
    if not skip_stage_1:
        ...
        for (input_path, output_path, mode, expected_len) in work_items:
            # 완전히 끝난 건 스킵
            if os.path.exists(output_path) and len(utils.read_jsonl(output_path)) == expected_len:
                continue

            # Stage1만 sample-wise
            run_single_work_item_stage1_samplewise(input_path, output_path, mode)
