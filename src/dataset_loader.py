...
# define the datasets
english_qa_datasets = ["lsat-ar", "lsat-lr", "lsat-rc", "logiqa-en", "sat-math", "sat-math-test", "sat-en", "aqua-rat",
...
def append_final_suffix_to_context(ctxt, chat_mode: bool):
    """
    context(ctxt)에 FINAL_SUFFIX를 붙이는 헬퍼.
    - chat_mode=False: ctxt는 문자열 → 문자열 뒤에 suffix를 그대로 붙임
    - chat_mode=True: ctxt는 role 기반 list일 수 있음 → 마지막 user 메시지에 suffix를 붙임
    """
    if chat_mode:
        # few-shot + chat_mode인 경우 ctxt는 role 리스트([{"role":..., "content":...}, ...])
        if isinstance(ctxt, list) and len(ctxt) > 0:
            # 마지막 user 메시지(content)에 suffix를 덧붙임
            last = ctxt[-1]
            if isinstance(last, dict) and "content" in last:
                last["content"] = last["content"] + FINAL_SUFFIX
            return ctxt
        else:
            # 혹시 문자열로 들어오면 그냥 문자열 뒤에 붙이기
            return str(ctxt) + FINAL_SUFFIX
    else:
        # 일반 문자열 context
        return str(ctxt) + FINAL_SUFFIX
...
def load_dataset(dataset_name, setting_name, parent_path, prompt_path=None, max_tokens=None, end_of_example="\n",
    ...
    for meta_idx, line in enumerate(loaded_jsonl):
        ...
        ctxt = append_final_suffix_to_context(ctxt, chat_mode)
